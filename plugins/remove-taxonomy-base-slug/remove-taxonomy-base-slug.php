...
function remove_taxonomy_base_slug__remove_post_type_filter( $post_type_rewrite ) {
	...
	$post_type = isset( $post_type_arr[1] ) ? sanitize_key( $post_type_arr[1] ) : '';
	if ( empty( $post_type ) ) {
	...
	foreach ( $taxonomies as $taxonomy ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( $post_type_object && ! empty( $post_type_object->rewrite['slug'] ) && $post_type_object->rewrite['slug'] == $term->slug ) {
